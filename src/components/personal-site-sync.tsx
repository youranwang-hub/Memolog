...
export function PersonalSiteSync() {
  ...
  return <Card>
    ...
    <CardContent className="flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-muted-foreground">当前有 {state.count} 条记忆会同步到个人网站。{state.configured ? "" : " GitHub 同步尚未配置。"}</p><Button variant="outline" onClick={sync} disabled={syncing || !state.configured}>{syncing ? <Loader2 className="mr-1 h-4 w-4 animate-spin" /> : <RefreshCw className="mr-1 h-4 w-4" />}立即同步公开记忆</Button></CardContent>
  </Card>;
}
